...
class _MyDialPadWidget extends State<DialPadWidget>
  ...
  Future<Widget?> _handleCall(BuildContext context,
    ...
    print("kIsWeb && !voiceOnly ${kIsWeb && !voiceOnly}");
    if (kIsWeb && !voiceOnly) {
      mediaStream =
      ...
      if (audioTracks.isNotEmpty) {
        await mediaStream.addTrack(audioTracks.first, addToNative: true);
      }
    } else {
      print("voiceOnly $voiceOnly");
      if (voiceOnly) {
        mediaConstraints['video'] = !voiceOnly;
      }
      mediaStream = await navigator.mediaDevices.getUserMedia(mediaConstraints);
    }
    await helper!.call(dest, voiceonly: voiceOnly, mediaStream: mediaStream);
  ...
  void callStateChanged(Call call, CallState callState) {
    if (callState.state == CallStateEnum.CALL_INITIATION) {
      Navigator.pushNamed(context, '/callscreen', arguments: {
        "call": call,
        "remote_render": _remoteRenderer,
        "local_render": _localRenderer,
      });
    }
  }
